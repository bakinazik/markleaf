...
function fallbackRootId(isMobileCtx) {
  if (isFirefox) return isMobileCtx ? 'mobile______' : 'toolbar_____';
  return isMobileCtx ? '3' : '1';
}

const FAVICON_CACHE_KEY = 'faviconCache';
let faviconCache = {};

const DEFAULT_FAVICON_ICON = 'data:image/svg+xml,' + encodeURIComponent(
  '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="#9aa0a6" stroke-width="1.5">' +
  '<circle cx="12" cy="12" r="10"/>' +
  '<line x1="2" y1="12" x2="22" y2="12"/>' +
  '<path d="M12 2c2.8 2.7 4.4 6.3 4.4 10s-1.6 7.3-4.4 10c-2.8-2.7-4.4-6.3-4.4-10s1.6-7.3 4.4-10z"/>' +
  '</svg>'
);
function faviconHostname(url) {
  try { return new URL(url).hostname; } catch { return null; }
}
function cachedFaviconUrl(host) {
  const entry = faviconCache[host];
  if (!entry) return null;
  return typeof entry === 'string' ? entry : entry.d;
}
function loadFaviconCache() {
  return new Promise(resolve => {
    chrome.storage.local.get([FAVICON_CACHE_KEY], (d) => {
      faviconCache = (d && d[FAVICON_CACHE_KEY]) || {};
      resolve(faviconCache);
    });
  });
}
...
function cacheFaviconForTab(tab) {
  ...
  if (!/^(https?:|data:)/.test(tab.favIconUrl)) return;
  console.log('[Markleaf][popup] sending cacheFaviconForTab for', tab.url);
  chrome.runtime.sendMessage({ type: 'cacheFaviconForTab', tab: { url: tab.url, favIconUrl: tab.favIconUrl } });
}
function scanOpenTabsForFavicons() {
  return new Promise(resolve => {
    chrome.tabs.query({}, (tabs) => {
      console.log('[Markleaf][popup] scanning', (tabs || []).length, 'open tabs for favicons');
      (tabs || []).forEach(cacheFaviconForTab);
      resolve();
    });
  });
}

function openBookmarkTab(url, { active = false, updateCurrent = false } = {}) {
  const originHost = faviconHostname(url);
  const onOpened = (tab) => {
    if (tab && tab.id != null && originHost) {
      console.log('[Markleaf][popup] opened tab', tab.id, 'for bookmark host', originHost);
      chrome.runtime.sendMessage({ type: 'trackBookmarkTab', tabId: tab.id, originHost });
    }
  };
  if (updateCurrent) {
    chrome.tabs.update({ url }, onOpened);
  } else {
    chrome.tabs.create({ url, active }, onOpened);
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  if (isFirefox) {
    ...
    loadFaviconCache().then((cache) => console.log('[Markleaf][popup] loaded favicon cache, hosts:', Object.keys(cache))).then(() => scanOpenTabsForFavicons());

    chrome.storage.onChanged.addListener((changes, area) => {
      if (area !== 'local' || !changes[FAVICON_CACHE_KEY]) return;
      console.log('[Markleaf][popup] faviconCache changed in storage, updating visible icons');
      faviconCache = changes[FAVICON_CACHE_KEY].newValue || {};
      document.querySelectorAll('img[data-favicon-host]').forEach((img) => {
        const host = img.dataset.faviconHost;
        const cachedUrl = host && cachedFaviconUrl(host);
  ...
  const ICONS = {
    ...
    libraryPlusSmall: (size = 14) => `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M7 7m0 2.667a2.667 2.667 0 0 1 2.667 -2.667h8.666a2.667 2.667 0 0 1 2.667 2.667v8.666a2.667 2.667 0 0 1 -2.667 2.667h-8.666a2.667 2.667 0 0 1 -2.667 -2.667z" /><path d="M4.012 16.737a2.005 2.005 0 0 1 -1.012 -1.737v-10c0 -1.1 .9 -2 2 -2h10c.75 0 1.158 .385 1.5 1" /></svg>`,
...
function renderTrashSelectBtnIcon() {
  ...
  function showBulkContextMenu(x, y, mode) {
    ...
    if (mode === 'bookmarks') {
      ...
      if (bookmarkNodes.length > 0) {
        ...
        openBtn.onclick = () => {
          bookmarkNodes.forEach(n => openBookmarkTab(n.url));
          closeBulkContextMenu();
          exitSelectionMode();
        };
  ...
  chooseCustomIconBtn.addEventListener('click', () => {
    chrome.tabs.create({ url: chrome.runtime.getURL('helper.html?mode=icon') });
  });

  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== 'local' || !changes.customIconData) return;
    const icons = changes.customIconData.newValue;
    if (!icons) return;
  ...
  function focusTabSearch(tab) {
    if (isMobile) return;
    if (tab === 'bookmarks') searchInput.focus();
    else if (tab === 'trash') trashSearchInput.focus();
    else if (tab === 'settings' && settingsSearchInput) settingsSearchInput.focus();
  }
  ...
  if (settingsPanel) {
    settingsPanel.addEventListener('change', (e) => {
      if (e.target.closest('.color-picker-popover, .custom-theme-swatches, #customFontArea')) return;
      setTimeout(refocusSettingsSearch, 0);
    });

    settingsPanel.addEventListener('click', (e) => {
      if (e.target === settingsSearchInput || e.target === clearSettingsSearchBtn) return;
      if (e.target.closest('select, input[type="file"], .color-picker-popover, .custom-theme-swatches, #customFontArea')) return;
      setTimeout(refocusSettingsSearch, 0);
    });
  }
  ...
  async function loadTrashPanel(filterTerm = '') {
    ...
    items.forEach(trashEntry => {
      ...
      if (trashEntry.url) {
        a.addEventListener('click', (event) => {
          ...
          if (event.ctrlKey || event.metaKey) {
            openBookmarkTab(trashEntry.url);
            return false;
          }
          if (event.shiftKey) {
          ...
          openBookmarkTab(trashEntry.url, { updateCurrent: true });
          return false;
        });
      ...
      if (trashEntry.url) {
        ...
        wireFaviconEvents(img, trashEntry.url);
        img.alt = '';
      ...
      const TRASH_QUICK_ACTIONS = {
        ...
        openInCurrentTab: {
          ...
          onClick: (e) => {
            e.stopPropagation();
            if (trashEntry.url) openBookmarkTab(trashEntry.url, { updateCurrent: true });
            closeMenu();
          }
        },
        openInNewTab: {
          ...
          onClick: (e) => {
            e.stopPropagation();
            if (trashEntry.url) openBookmarkTab(trashEntry.url);
            closeMenu();
          }
        }
      };
...
function updateSettingsUI() {
  ...
  populateRootFolderSelect(folderStack[0] || fallbackRootId(isMobile));
  ...
  const DEFAULT_CUSTOM_THEME_COLORS = { bg: '#ffffff', text: '#111111', accent: '#00b017', border: '#dddddd', card: '#f0f0f5' };
  const swatchEls = {
    bg:     document.getElementById('swatchBg'),
    text:   document.getElementById('swatchText'),
    accent: document.getElementById('swatchAccent'),
    border: document.getElementById('swatchBorder'),
    card:   document.getElementById('swatchCard'),
  };
  const swatchBtnEls = {
    bg:     document.getElementById('swatchBtnBg'),
    text:   document.getElementById('swatchBtnText'),
    accent: document.getElementById('swatchBtnAccent'),
    border: document.getElementById('swatchBtnBorder'),
    card:   document.getElementById('swatchBtnCard'),
  };
  let currentCustomColors = Object.assign({}, DEFAULT_CUSTOM_THEME_COLORS);

  function updateThemeSwatches(c) {
    const colors = Object.assign({}, DEFAULT_CUSTOM_THEME_COLORS, c || {});
    Object.keys(swatchEls).forEach(key => {
      swatchEls[key].style.setProperty('--swatch-color', colors[key]);
    });
  }
  ...
  themeSelect.addEventListener('change', () => {
    ...
    if (val === 'custom') {
      chrome.storage.local.get(['customThemeColors'], (d) => {
        const colors = Object.assign({}, DEFAULT_CUSTOM_THEME_COLORS, d.customThemeColors || {});
        currentCustomColors = colors;
        updateThemeSwatches(colors);
        applyTheme('custom', colors);
        chrome.storage.local.set({ appTheme: val, customThemeColors: colors });
        closeColorPicker();
      });
    } else {
      applyTheme(val, null);
  ...
  const cpPopover = document.getElementById('colorPickerPopover');
  const cpActiveLabel = document.getElementById('cpActiveLabel');
  const cpSvWrap = document.getElementById('cpSvWrap');
  const cpSvCanvas = document.getElementById('cpSvCanvas');
  const cpSvThumb = document.getElementById('cpSvThumb');
  const cpHueWrap = document.getElementById('cpHueWrap');
  const cpHueThumb = document.getElementById('cpHueThumb');
  const cpHexInput = document.getElementById('cpHexInput');
  const cpSwatchPreview = document.getElementById('cpSwatchPreview');

  const SWATCH_LABEL_KEYS = {
    bg: 'settingThemeBgColor',
    text: 'settingThemeTextColor',
    accent: 'settingThemeAccentColor',
    border: 'settingThemeBorderColor',
    card: 'settingThemeCardColor',
  };
  ...
  function positionThumbs() {
    cpSvThumb.style.left = (hsv.s * 100) + '%';
    cpSvThumb.style.top = ((1 - hsv.v) * 100) + '%';
    cpHueThumb.style.left = (hsv.h / 360 * 100) + '%';
  }
  ...
  function refreshPreview() {
    ...
    cpSwatchPreview.style.setProperty('--swatch-color', hex);
    cpHexInput.value = hex.slice(1).toUpperCase();
    cpSvThumb.style.background = hex;
    cpHueThumb.style.background = `hsl(${hsv.h}, 100%, 50%)`;
    return hex;
  }

  function commitActiveColor() {
    if (!activeColorKey) return;
    const hex = refreshPreview();
    currentCustomColors[activeColorKey] = hex;
    swatchEls[activeColorKey].style.setProperty('--swatch-color', hex);
    applyTheme('custom', currentCustomColors);
    chrome.storage.local.set({ appTheme: 'custom', customThemeColors: currentCustomColors });
  }

  function openColorPicker(key) {
    activeColorKey = key;
    Object.keys(swatchBtnEls).forEach(k => swatchBtnEls[k].classList.toggle('active', k === key));
    const hex = currentCustomColors[key] || DEFAULT_CUSTOM_THEME_COLORS[key];
    const rgb = hexToRgb(hex);
    hsv = rgbToHsv(rgb.r, rgb.g, rgb.b);
    cpActiveLabel.textContent = chrome.i18n.getMessage(SWATCH_LABEL_KEYS[key]) || '';
    cpPopover.style.display = 'flex';
    drawSvCanvas(hsv.h);
    positionThumbs();
    refreshPreview();
  }
  ...
  function closeColorPicker() {
    activeColorKey = null;
    Object.keys(swatchBtnEls).forEach(k => swatchBtnEls[k].classList.remove('active'));
    cpPopover.style.display = 'none';
  }

  Object.keys(swatchBtnEls).forEach(key => {
    swatchBtnEls[key].addEventListener('click', () => {
  ...
  function huePointFromEvent(e) {
    const rect = cpHueWrap.getBoundingClientRect();
    const x = Math.max(0, Math.min(rect.width, e.clientX - rect.left));
  ...
  cpSvWrap.addEventListener('pointerup', (e) => {
    draggingSv = false;
    try { cpSvWrap.releasePointerCapture(e.pointerId); } catch (err) {}
  });

  cpHueWrap.addEventListener('pointerdown', (e) => {
    if (!activeColorKey) return;
    draggingHue = true;
    cpHueWrap.setPointerCapture(e.pointerId);
    hsv.h = huePointFromEvent(e);
    drawSvCanvas(hsv.h);
    positionThumbs();
    commitActiveColor();
  });
  cpHueWrap.addEventListener('pointermove', (e) => {
  ...
  cpHueWrap.addEventListener('pointerup', (e) => {
    draggingHue = false;
    try { cpHueWrap.releasePointerCapture(e.pointerId); } catch (err) {}
  });

  cpHexInput.addEventListener('input', () => {
    const raw = cpHexInput.value.replace(/[^0-9a-fA-F]/g, '').slice(0, 6);
    if (raw.length !== cpHexInput.value.length) cpHexInput.value = raw;
    if (raw.length !== 6 || !activeColorKey) return;
  ...
  document.addEventListener('click', (e) => {
    if (!activeColorKey) return;
    if (cpPopover.contains(e.target)) return;
    if (Object.values(swatchBtnEls).some(btn => btn.contains(e.target))) return;
    closeColorPicker();
  });

  const fontSelect = document.getElementById('fontSelect');
  const customFontArea = document.getElementById('customFontArea');
  const customFontInput = document.getElementById('customFontInput');
  let customFontStyleEl = null;
  let customFontTimer = null;

  function applyFontFamily(font, customName, customDataUrl) {
    const html = document.documentElement;
    if (font === 'custom' && customName && customDataUrl) {
      if (!customFontStyleEl) {
        customFontStyleEl = document.createElement('style');
        document.head.appendChild(customFontStyleEl);
      }
      customFontStyleEl.textContent = `@font-face { font-family: "${customName}"; src: url("${customDataUrl}"); }`;
      html.style.setProperty('--font-family', `"${customName}", sans-serif`);
    } else {
      if (customFontStyleEl) customFontStyleEl.textContent = '';
      html.style.setProperty('--font-family', font || 'sans-serif');
    }
  }

  async function fetchAndApplyBunnyFont(name) {
    const slug = name.trim().toLowerCase().replace(/\s+/g, '-');
    if (!slug) return;
    try {
      const cssRes = await fetch(`https://fonts.bunny.net/css?family=${encodeURIComponent(slug)}:400`);
      if (!cssRes.ok) return;
      const cssText = await cssRes.text();
      const matches = [...cssText.matchAll(/url\(([^)]+)\)\s*format\('([^']+)'\)/g)];
      if (!matches.length) return;
      const woff2 = matches.find(m => m[2] === 'woff2') || matches[0];
      const fontUrl = woff2[1].replace(/^['"]|['"]$/g, '');
      ...
      applyFontFamily('custom', name, dataUrl);
      chrome.storage.local.set({ fontFamily: 'custom', customFontName: name, customFontDataUrl: dataUrl });
    } catch (err) {
      console.error('Bunny font fetch error:', err);
    }
  }

  fontSelect.addEventListener('change', () => {
    const val = fontSelect.value;
    customFontArea.style.display = val === 'custom' ? 'flex' : 'none';
    if (val === 'custom') {
      chrome.storage.local.get(['customFontName', 'customFontDataUrl'], (d) => {
        if (d.customFontName) customFontInput.value = d.customFontName;
        if (d.customFontName && d.customFontDataUrl) {
          applyFontFamily('custom', d.customFontName, d.customFontDataUrl);
          chrome.storage.local.set({ fontFamily: 'custom' });
        }
      });
    } else {
      applyFontFamily(val, null, null);
      chrome.storage.local.set({ fontFamily: val });
    }
  });

  customFontInput.addEventListener('input', () => {
    clearTimeout(customFontTimer);
    const val = customFontInput.value;
    customFontTimer = setTimeout(() => fetchAndApplyBunnyFont(val), 200);
  });
  ...
  function populateRootFolderSelect(selectedId) {
    ...
    chrome.bookmarks.getChildren(isFirefox ? 'root________' : '0', (rootChildren) => {
    ...
    chrome.bookmarks.getSubTree(node.id, (subNodes) => {
      ...
      if (listItem._collageCells && isFolderStackMode) {
        ...
        listItem._collageCells.forEach((cell, i) => {
          ...
          wireFaviconEvents(img, faviconUrls[i]);
          img.alt = '';
          img.loading = 'lazy';
  ...
  function resolveValidRootFolderId(preferredId, cb) {
    chrome.bookmarks.getTree((tree) => {
      ...
      const guess = fallbackRootId(isMobile);
...
async function showEmptyAreaContextMenu(x, y) {
  ...
  addCurrentBtn.onclick = () => {
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      const tab = tabs[0];
      if (!tab || isInternalBrowserUrl(tab.url)) return;
  ...
  addAllBtn.onclick = async () => {
    ...
    for (const tab of tabs) {
      if (isInternalBrowserUrl(tab.url)) continue;
      cacheFaviconForTab(tab);
  ...
  function createListItem(node, precomputedData = {}) {
    ...
    const withCacheBust = (u) => u.startsWith('data:') ? u : `${u}${u.includes('?') ? '&' : '?'}retry=${Date.now()}`;
      ...
      [1000, 2000, 3000].forEach((delay) => {
        setTimeout(() => {
          faviconTargets.forEach(({ img, size }) => {
            img.src = withCacheBust(getFaviconUrl(node.url, size));
          });
        }, delay);
      });
    };
    ...
    if (!node.url) {
      ...
      link.addEventListener('click', (event) => {
        ...
        if (event.ctrlKey || event.metaKey) { openBookmarkTab(node.url); return false; }
        if (event.shiftKey) { chrome.windows.create({ url: node.url }); return false; }
        openBookmarkTab(node.url, { updateCurrent: true }); return false;
      });
      link.addEventListener('auxclick', (event) => {
        if (event.button === 1) {
          event.preventDefault(); event.stopPropagation();
          scheduleFaviconRefresh();
          openBookmarkTab(node.url); return false;
        }
      });
    }
    ...
    if (node.url) {
      ...
      wireFaviconEvents(img, node.url);
      img.alt = 'Favicon';
      img.loading = 'lazy';
      img.style.cssText = 'width:16px;height:16px;margin-right:8px;border-radius:100%;';
    ...
    if (node.url) {
      ...
      wireFaviconEvents(gridImg, node.url);
      gridImg.alt = '';
    ...
    const ITEM_DEFINITIONS = {
      ...
      copyLink: {
        icon: ICONS.libraryPlusSmall(16),
        label: chrome.i18n.getMessage('copyLinkButton'),
        onClick: (event, btn, isQuick) => {
          event.stopPropagation(); closeMenu();
      ...
      openInCurrentTab: {
        ...
        onClick: (event, btn, isQuick) => {
          event.stopPropagation(); closeMenu();
          openBookmarkTab(node.url, { updateCurrent: true });
        }
      },
      openInNewTab: {
        icon: ICONS.openExternal(14),
  ...
  function openAllBookmarksInFolder(folderId) {
    function openBookmarksRecursive(nodeId) {
      chrome.bookmarks.getChildren(nodeId, (children) => {
        if (children) {
          children.forEach((child) => {
            if (child.url) openBookmarkTab(child.url);
  ...
  function getFaviconUrl(url, size = 16) {
    ...
    if (provider === 'browser' && isFirefox) {
      const host = faviconHostname(url);
      const cachedUrl = host && cachedFaviconUrl(host);
      if (cachedUrl) return cachedUrl;
      console.log('[Markleaf][popup] no cached favicon yet for', host, '— showing placeholder');

      scanOpenTabsForFavicons();
  ...
  function wireFaviconEvents(img, url) {
    if (!isFirefox) return;
    const host = faviconHostname(url);
    if (host) img.dataset.faviconHost = host;
    img.addEventListener('error', () => {
      if (img.src !== DEFAULT_FAVICON_ICON) img.src = DEFAULT_FAVICON_ICON;
    });
  }
  ...
  chrome.bookmarks.onRemoved.addListener((removedId) => {
    ...
    if (stackIdx !== -1) {
      ...
      if (trimmed.length > 0) {
        ...
        chrome.storage.local.get(['defaultFolderId'], (data) => {
          const fallbackId = data.defaultFolderId || fallbackRootId(isMobile);
          folderStack = [fallbackId];
          currentFolderId = fallbackId;
          chrome.storage.local.set({ folderStack });
          listItems(currentFolderId);
  ...
  chrome.storage.local.get(['folderStack', 'lastVisitedFolderStack', 'isGridView', 'isFolderStackMode', 'appTheme', 'customThemeColors', 'fontFamily', 'customFontName', 'customFontDataUrl', 'defaultFolderId', 'deleteMode', 'trashExpiryDays', 'showFolderBadge', 'checkBeforeAdd', 'ctrlDragMoveEnabled', 'showUpButton', 'showScrollTopButton', 'showBreadcrumb', 'showBreadcrumbPersist', 'showScrollbar', 'searchInFolder', 'ctxMenuItems', 'showQuickActions', 'gridColumns', 'gridWidth', 'showGridTitles', 'pinnedBookmarks', 'newItemPosition'], (data) => {
    ...
    if (data.folderStack && data.folderStack.length > 0) {
      ...
      const defaultId = data.defaultFolderId || fallbackRootId(isMobile);
    ...
    if (theme === 'custom') {
      ...
      currentCustomColors = Object.assign({}, DEFAULT_CUSTOM_THEME_COLORS, data.customThemeColors || {});
      updateThemeSwatches(currentCustomColors);
      applyTheme('custom', currentCustomColors);
    } else {
      applyTheme(theme);
    }
    ...
    if (font === 'custom') {
      customFontArea.style.display = 'flex';
      if (data.customFontName) customFontInput.value = data.customFontName;
      if (data.customFontName && data.customFontDataUrl) applyFontFamily('custom', data.customFontName, data.customFontDataUrl);
    } else {
      applyFontFamily(font, null, null);
    }
    ...
    tabTrash.style.display = deleteMode === 'trash' ? '' : 'none';
    setToggleIcon();
    const resolvedDefaultId = data.defaultFolderId || fallbackRootId(isMobile);
  ...
  document.addEventListener('keydown', e => {
    ...
    if (isGridView) {
      ...
      if (e.key === 'ArrowDown') {
        ...
        const newIdx = keyboardSelectedIndex < 0
          ? 0
          : Math.min(keyboardSelectedIndex + gridColumns, items.length - 1);
      ...
      if (e.key === 'ArrowUp') {
        ...
        const items = getVisibleItems();
        if (items.length === 0) return;
        const newIdx = keyboardSelectedIndex - gridColumns;
        if (newIdx < 0) {
          clearKeyboardSelection();
          !isMobile && searchInput.focus();
        } else {
      ...
      if (e.key === 'ArrowRight') {
        ...
        const newIdx = keyboardSelectedIndex < 0
          ? 0
          : Math.min(keyboardSelectedIndex + 1, items.length - 1);
        setKeyboardSelection(newIdx);
        return;
      }
      if (e.key === 'ArrowLeft') {
...
`;
  ...
  async function importBookmarksFromHtml(text) {
    try {
  ...
  async function exportData(options) {
    ...
    const exportObj = {
      ...
      settings: includeSettings ? {
        ...
        fontFamily: storageData.fontFamily || 'sans-serif',
        customFontName: (storageData.fontFamily === 'custom') ? (storageData.customFontName || null) : null,
        customFontDataUrl: (storageData.fontFamily === 'custom') ? (storageData.customFontDataUrl || null) : null,
  ...
  async function importData(text, options) {
    try {
      const includeSettings = !options || options.includeSettings !== false;
      const includeBookmarks = !options || options.includeBookmarks !== false;
      ...
      if (s.appTheme) {
        ...
        if (s.appTheme === 'custom' && s.customThemeColors) {
          customThemeArea.style.display = 'flex';
          currentCustomColors = Object.assign({}, DEFAULT_CUSTOM_THEME_COLORS, s.customThemeColors || {});
          updateThemeSwatches(currentCustomColors);
          applyTheme('custom', currentCustomColors);
          chrome.storage.local.set({ customThemeColors: currentCustomColors });
        } else {
      ...
      if (s.fontFamily) {
        ...
        if (s.fontFamily === 'custom' && s.customFontName && s.customFontDataUrl) {
          customFontArea.style.display = 'flex';
          customFontInput.value = s.customFontName;
          applyFontFamily('custom', s.customFontName, s.customFontDataUrl);
        } else {
          customFontArea.style.display = 'none';
          applyFontFamily(s.fontFamily, null, null);
        }
      }
  ...
  if (importBtn) importBtn.addEventListener('click', async () => {
    ...
    if (options) {
      const kind = isBookmarksOnly(options) ? 'bookmarksOnly' : 'full';
      chrome.storage.local.set({ pendingImportOptions: options }, () => {
        chrome.tabs.create({ url: chrome.runtime.getURL(`helper.html?mode=import&kind=${kind}`) });
      });
    }
  });

  async function resumePendingImportIfAny() {
    const data = await new Promise(resolve => chrome.storage.local.get(['pendingImportPayload', 'pendingImportOptions'], resolve));
    const payload = data.pendingImportPayload;
    if (!payload || !payload.text) return;
    await new Promise(resolve => chrome.storage.local.remove(['pendingImportPayload'], resolve));
    pendingImportOptions = data.pendingImportOptions || null;
    if (payload.kind === 'bookmarksOnly') {
      importBookmarksFromHtml(payload.text);
    } else {
      importData(payload.text, pendingImportOptions);
    }
  }
  resumePendingImportIfAny();

  chrome.storage.onChanged.addListener((changes, area) => {
    if (area !== 'local' || !changes.pendingImportPayload || !changes.pendingImportPayload.newValue) return;
    resumePendingImportIfAny();
  });

  if (importFileInput) importFileInput.remove();
  ...
  const setI18nTexts = () => {
    addBookmarkBtn.title = chrome.i18n.getMessage('addBookmark');
